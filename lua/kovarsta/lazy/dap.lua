...
return {
...

    {
        ...
        config = function()
			...
			--[[ These lines closes DAP when it finished running
	...
	
	{
		...
		config = function()
			-- 1. Identify the correct path to the debugpy venv created by Mason
			local mason_path = vim.fn.stdpath("data") .. "/mason/packages/debugpy/venv/"
			local python_exe = ""

			if vim.fn.has("win32") == 1 then
				python_exe = mason_path .. "Scripts/python.exe"
			else
				python_exe = mason_path .. "bin/python"
			end

			-- 2. Pass the absolute path to the debugpy python, NOT just "python"
			require('dap-python').setup(python_exe)
			
			-- Optional: Standardize the console to avoid the "Content-Length" terminal popup
